...
#RUN mkdir -p /opt/r-studio &&     mkdir -p /opt/r-lang &&     wget -O /opt/rstudio.tar.gz https://github.com/rstudio/rstudio/archive/v$RSTUDIO_VERSION.tar.gz &&     wget -O /opt/R.tar.gz       https://cran.r-project.org/src/base/R-3/R-$R_VERSION.tar.gz &&     tar -xf /opt/rstudio.tar.gz -C /opt/r-studio &&     tar -xf /opt/R.tar.gz       -C /opt/r-lang &&     apt-get update &&     cd /opt/r-studio/*/dependencies/linux/ &&     ./install-dependencies-debian --exclude-qt-sdk &&     apt-get install -y gfortran libreadline-dev libxt-dev liblzma-dev openjdk-8-jdk &&     update-java-alternatives -s java-1.8.0-openjdk-amd64 &&     cd /opt/r-lang/*/ &&     ln -s /lib/x86_64-linux-gnu/libreadline.so.7 /lib/x86_64-linux-gnu/libreadline.so.8 &&     LDFLAGS='-L/opt/conda/lib -licui18n -licuuc -licudata' ./configure --enable-R-shlib=yes --with-blas --with-lapack &&     make &&     make install &&     cd /opt/r-studio/*/ &&     mkdir build && cd build  &&     cmake .. -DRSTUDIO_TARGET=Server -DCMAKE_BUILD_TYPE=Release &&     make install &&     useradd -r rstudio-server &&     mkdir -p /var/run/rstudio-server &&     mkdir -p /var/lock/rstudio-server &&     mkdir -p /var/log/rstudio-server &&     mkdir -p /var/lib/rstudio-server &&     ln -f -s /usr/local/lib/rstudio-server/bin/rserver /usr/sbin/rserver &&     ln -f -s /usr/local/lib/rstudio-server/bin/rsession /usr/sbin/rsession &&     cp /usr/local/lib/R/lib/*.so /usr/lib &&     mkdir -p /etc/rstudio &&     touch /etc/rstudio/repos.conf

COPY install.R /opt/

#RUN R CMD javareconf &&     apt-get update && apt-get install -y libzmq3-dev libssl-dev libcurl4-openssl-dev libxml2-dev &&     ln -sf /usr/lib/x86_64-linux-gnu/libicui18n.so.64.2 /usr/lib/x86_64-linux-gnu/libicui18n.so.64 &&     ln -sf /usr/lib/x86_64-linux-gnu/libicui18n.so.64.2 /usr/lib/x86_64-linux-gnu/libicui18n.so &&     Rscript /opt/install.R --save &&     chown -R efx_container_user /usr/local/lib/R/ &&     Rscript -e "installed.packages()" > /opt/installed.packages.txt &&     cat /opt/installed.packages.txt
